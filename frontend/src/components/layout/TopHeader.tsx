...
export function TopHeader({ title, subtitle, actions, contextInfo, onOpenSidebar }: TopHeaderProps) {
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/85">
      <div className="mx-auto flex w-full max-w-[88rem] items-start justify-between gap-2 px-4 py-4 sm:px-6 md:gap-4 lg:px-8">
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h1 className="min-w-0 truncate text-xl font-semibold tracking-tight md:text-2xl" title={title}>{title}</h1>
          </div>
